    ...
    <style>
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        input[type="text"], input[type="file"], select, textarea {
            width: 100%; padding: 12px 15px; border: 1px solid var(--border-color); border-radius: var(--border-radius); font-family: 'Poppins', sans-serif; font-size: 1rem; transition: border-color 0.3s;
        }
            ...
            <select id="targets" name="targets[]" multiple required>
                ...
                <optgroup label="Students">
                    ...
                    <?php while ($s26 = $students26->fetch_assoc()) { ?>
                        <option value="student26_<?= $s26['id'] ?>">Student26: <?= htmlspecialchars($s26['name']) ?></option>
                    <?php } ?>
                </optgroup>
            </select>
        </div>
